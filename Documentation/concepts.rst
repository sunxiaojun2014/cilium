...
* **Restoring:** The endpoint is being restored from previous state and has not
  been successfully built yet. The connectivity state of the endpoint depends
  on the endpoint state while the previous instance of the Cilium agent was
  managing the endpoint. The endpoint will transition to Ready once the restore
